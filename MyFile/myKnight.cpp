...
#define LOG_STAT
#else
#define LOG_STAT cout << theKnight.HP << ' ' << theKnight.level << ' ' << theKnight.remedy << ' ' << theKnight.maidenkiss << ' ' << theKnight.phoenixdown << endl
#endif
...
int main(int argc, char** argv)
{
	...
	for (i = 0; i < nEvent; i++)
	{
		...
		bool normalWin = theKnight.level > levelO && weapon != Excalipoor || isArthur || isPaladin || isDragonKnight && weapon == DragonBlade || odinBuff;
		...
		break;
		case 10:	// EXCALIPOOR
			if (theKnight.level < 5 && !isArthur && !isPaladin && !isDragonKnight)
				weapon = Excalipoor;
		break;
		...
		break;
		case 13:	// MUSH GHOST
			if ( !(isPaladin || isDragonKnight && weapon == DragonBlade || armor == Mithril) ) 
			{ 
		...
		break;
		case 18:	// MERLIN
			if (isTiny) { theKnight.remedy++; }
			if (isFrog) { theKnight.maidenkiss++; }
		...
		case 19:	// ABYSS
			if (theKnight.level < 7 && !(isDragonKnight && weapon == DragonBlade)) { *nOut = -1; }
		break;
		...
		case 20:	// PRINCESS ESCAPED == Past become future
			...
			checkPoint = i + 1;
			past = 0;
			nEvent = 2 * checkPoint - 1;

			for (int future = nEvent - 1; future > checkPoint - 1; future--) 
			{	
		...
		case 99:	// BOWSER
			if (odinBuff || isArthur || isLancelot || isPaladin && theKnight.level > 7 || theKnight.level == 10 || isDragonKnight && weapon == DragonBlade)
			{
				theKnight.level = 10;
			}
			else
